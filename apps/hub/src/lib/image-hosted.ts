/**
 * 호스팅 이미지 생성 — 사용자 Gemini 키로 이미지를 생성하고 Vercel Blob 에 저장해 공개 URL 반환.
 * Pollinations 무료가 IP 큐 제한(402)으로 막혀, BYOK Gemini 이미지 생성으로 대체.
 */

import { put } from '@vercel/blob';
import { getUserApiKey, type ImageRatio } from '@amakers/ai';
...
export async function generateImageHosted(userId: string, prompt: string, ratio: ImageRatio = 'square'): Promise<HostedImageResult> {
  ...
  if (!key) {
    return { ok: false, error: '이미지 생성에는 Gemini 키가 필요해요(무료 Pollinations가 막힘). 설정(키)에서 Gemini 키를 등록하세요.' };
  }
