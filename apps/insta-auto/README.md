# instaauto — 인스타그램 자동화봇

마케팅봇에서 인스타그램 기능만 떼어낸 **플랫폼 전용 자동화봇**. amakers 멀티봇 플랫폼의 한 앱(`apps/insta-auto`)으로, 공통 인증·결제·DB 를 공유한다.

- **도메인**: instaauto.amakers.co.kr (예정)
...
# 모노레포 루트에서
npm install
npm run db:generate              # (또는 cd apps/insta-auto && npx prisma generate)
