...
const PROVIDERS = [
    { id: 'gemini', name: 'Google Gemini', help: 'aistudio.google.com/app/apikey', desc: '무료 티어 — 캡션 생성용' },
    { id: 'groq', name: 'Groq', help: 'console.groq.com/keys', desc: '무료 티어 — 빠른 캡션 생성' },
];
...
export function ApiKeysForm({ initialKeys }: { initialKeys: KeyRow[] }) {
    ...
    const save = (provider: string) => {
        ...
        startTransition(async () => {
            ...
            if (r.ok) {
                notifications.show({ title: '키 저장', message: `${provider} 키가 등록되었습니다 (${r.maskedHint})`, color: 'teal' });
    ...
    return (
        <Stack gap="md">
            <Alert variant="light" color="grape" icon={<IconInfoCircle size={16} />}>
                <Text size="sm">
                    AI 캡션 생성을 쓰려면 <strong>무료 API 키</strong> 하나를 등록하세요. 키는 AES-256-GCM 으로 암호화 저장되며, 본인만 사용합니다.
                    (Gemini 우선 사용, 없으면 Groq)
                </Text>
            </Alert>
                ...
                return (
                    <Card key={p.id} withBorder p="md" radius="md">
                        <Group justify="space-between" mb="xs">
                            ...
                            <Anchor href={`https://${p.help}`} target="_blank" rel="noreferrer" size="xs">키 발급 ↗</Anchor>
                        </Group>
                        ...
                        <Group gap="xs" align="flex-end">
                            ...
                            />
                            <Button color="grape" loading={pending} onClick={() => save(p.id)} disabled={!(inputs[p.id] || '').trim()}>저장</Button>
