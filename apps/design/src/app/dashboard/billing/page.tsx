...
import {
    AppShell, Container, Title, Text, Stack, Group, Card, Badge, Button,
    ThemeIcon, Box, Paper, Table, Anchor,
} from '@mantine/core';
import { IconBrush, IconCoin, IconArrowLeft, IconShoppingCart } from '@tabler/icons-react';
...
const ACTION_LABELS: Record<string, { label: string; color: string }> = {
    ...
    OUTLINE:    { label: 'AI 레이아웃',      color: 'grape' },
    PUBLISH:    { label: '발행',             color: 'green' },
...
export default async function BillingPage() {
    ...
    const purchased = transactions.filter((t: any) => t.delta > 0).reduce((s: number, t: any) => s + t.delta, 0);
    const used = transactions.filter((t: any) => t.delta < 0).reduce((s: number, t: any) => s + Math.abs(t.delta), 0);
                    ...
                    <Paper withBorder p="lg" radius="md" mb="lg">
                        <Group gap="xl">
                            <Box>
                                ...
                                <Text size="xs" c="dimmed">모든 봇 공유 잔액</Text>
                            </Box>
                            <Box>
                                ...
                                <Table.Tbody>
                                    {transactions.map((t: any) => {
                                        ...
                                        return (
                                            <Table.Tr key={t.id}>
                                                <Table.Td>
                                                    <Text size="xs">{dayjs(t.createdAt).format('YYYY-MM-DD HH:mm')}</Text>
                                                </Table.Td>
                                                <Table.Td>
