...
import {
    IconSend, IconCalendarTime, IconAlertCircle, IconSparkles, IconClock, IconArticle, IconWand, IconPhoto, IconStar,
...
export function ComposeForm({
    ...
    const genImage = () => {
        ...
        startAi(async () => {
            ...
            if (r.ok && r.url) {
                setLastImage(r.url);
                notifications.show({ title: 'AI 이미지 생성', message: '아래에서 대표/본문 삽입을 선택하세요.', color: 'blue' });
            } else {
    ...
    const submit = (publishNow: boolean) => {
        ...
        startTransition(async () => {
            ...
            const msg = publishNow
                ? `${r.published}/${r.accounts}개 블로그 발행 완료${r.failed ? ` · 실패 ${r.failed}` : ''}`
                : `${r.accounts}개 블로그에 ${schedule ? '예약' : '초안 저장'} 완료`;
            notifications.show({ title: publishNow ? '발행' : schedule ? '예약' : '저장', message: msg, color: r.failed ? 'orange' : 'teal' });
            router.push('/dashboard');
        });
    };
    ...
    return (
        <Group align="flex-start" gap="lg" wrap="wrap">
            ...
            <Card withBorder p="lg" radius="md" style={{ flex: 1, minWidth: 380 }}>
                <Stack>
                    ...
                    <Paper withBorder p="sm" radius="md" bg="blue.0">
                        <Group gap="xs" mb={6}><IconWand size={16} color="var(--mantine-color-blue-6)" /><Text size="sm" fw={700}>AI 글 작성 (주제 한 줄 → 제목+본문)</Text></Group>
                        <Stack gap="xs">
                            <TextInput placeholder="주제 예: 강아지 수제 간식 만드는 법" value={topic} onChange={(e) => setTopic(e.currentTarget.value)} />
                    ...
                    <Paper withBorder p="sm" radius="md" bg="blue.0">
                        ...
                        <Stack gap="xs">
                            <TextInput placeholder="예: 가을 캠핑, 모닥불, 따뜻한 분위기 사진" value={aiPrompt} onChange={(e) => setAiPrompt(e.currentTarget.value)} />
                            ...
                            {lastImage && (
                                <Group gap="xs">
                                    ...
                                    <Button size="xs" variant="light" color="blue" leftSection={<IconStar size={13} />} onClick={() => { setPhotoUrl(lastImage); notifications.show({ message: '대표 이미지로 설정', color: 'blue' }); }}>대표로</Button>
                    ...
                    <Group justify="flex-end">
                        {schedule ? (
                            ...
                            <>
                                ...
                                <Button color="blue" leftSection={<IconSend size={16} />} loading={pending} onClick={() => submit(true)}>{isEdit ? '수정 후 발행' : '지금 발행'}</Button>
                            </>
                        )}
                    </Group>
                </Stack>
            ...
            <Box style={{ width: 400 }}>
                ...
                <Group gap={6} mt="xs">
                    <ThemeIcon size="xs" variant="light" color="blue"><IconArticle size={12} /></ThemeIcon>
                    <Text size="11px" c="dimmed">발행 시 마크다운이 워드프레스 HTML로 변환됩니다.</Text>
                </Group>
            </Box>
        </Group>
    );
