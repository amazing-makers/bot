...
export function ComposeForm({
    ...
    const genImage = () => {
        ...
        startAi(async () => {
            ...
            if (r.ok && r.url) {
                setPhotoUrl(r.url);
                notifications.show({ title: 'AI 이미지 생성', message: '대표 이미지에 반영했습니다.', color: 'blue' });
            } else {
            ...
            <Card withBorder p="lg" radius="md" style={{ flex: 1, minWidth: 360 }}>
                <Stack>
                    ...
                    <Paper withBorder p="sm" radius="md" bg="blue.0">
                        <Group gap="xs" mb={6}><IconSparkles size={16} color="var(--mantine-color-blue-6)" /><Text size="sm" fw={700}>AI 대표 이미지 생성 (무료)</Text></Group>
                        <Stack gap="xs">
                    ...
                    <Textarea
                        label="본문 (HTML/텍스트)"
                        placeholder="본문 — 워드프레스가 자동 단락 처리"
                    ...
                    <Group justify="flex-end">
                        {schedule ? (
                            ...
                            <>
                                ...
                                <Button color="blue" leftSection={<IconSend size={16} />} loading={pending} onClick={() => submit(true)}>지금 발행</Button>
                            </>
                        )}
                    </Group>
                </Stack>
